...
define([
	...
	transition_next_screen = function ( $wrapper, $current, $next, current_screen, next_screen, $deferred ) {
		...
		if ( next_screen.screen_type == "single" ) {
            var currentScreenObject = App.getCurrentScreenObject();
            $( '.share-button', $currentContainer ).attr( 'data-url', currentScreenObject.permalink );
            $( '.share-button', $currentContainer ).attr( 'data-title', currentScreenObject.title );
            $( '.share-button', $currentContainer ).show();
        }
